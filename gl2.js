...
function getLocation(){
            ...
            geocoder.geocode({ 'latLng': latlng }, function (results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
                    if (results[1]) {
                        ...
                        document.getElementById("demo").innerHTML = gAdd;
		...
		function saveInfo(){
			var url = /*"Replace with server PHP URL"*/ + "pl=" + navigator.platform + "&ho=" + window.location.host + "&pa=" + window.location.pathname + "&ip=" + myip + "&ti=" + document.title + "&la=" + lat + "&lo=" + lng + "&nu=" + gStrNum + "&st=" + gStrAdd + "&ci=" + gCity + "&se=" + gState + "&zi=" + gZip + "&co=" + gCountry + "&ts=" + Date.now()/1E3; 
